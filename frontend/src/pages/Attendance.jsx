...
function parseSessions(sessions = []) {
  if (!sessions.length) return { checkIn: null, checkOut: null, isActive: false };
  const sorted = [...sessions].sort((a, b) => new Date(a.login_time) - new Date(b.login_time));
  const checkIn = sorted[0]?.login_time || null;
  const completed = sorted.filter(s => s.logout_time);
  const checkOut = completed.length ? completed[completed.length - 1].logout_time : null;
  const isActive = sorted.some(s => !s.logout_time);
  return { checkIn, checkOut, isActive };
}
...
export default function Attendance() {
  ...
  const [adminDate, setAdminDate] = useState(toIsoDate(new Date()));
  const [loading, setLoading] = useState(true);
  const isEmployee = user.role === 'employee';
  const isAdminView = hasRole('admin', 'hr_officer', 'payroll_officer') && !isEmployee;
  ...
  const fetchData = useCallback(async () => {
    try {
      if (isEmployee) {
        ...
        const selected = new Date(adminDate + 'T00:00:00');
        const selectedMonth = selected.getMonth() + 1;
        const selectedYear = selected.getFullYear();

        const [employeesRes, allRes, statusRes] = await Promise.all([
          API.get('/employees/'),
          API.get(`/attendance/all?month=${selectedMonth}&year=${selectedYear}`),
          adminDate === toIsoDate(new Date()) ? API.get('/employees/status/all') : Promise.resolve({ data: [] }),
        ]);

        const statusMap = new Map(statusRes.data.map(e => [e.id, e.attendance_status]));
        const recordsByEmp = new Map();
        allRes.data
          .filter(r => r.date === adminDate)
          .forEach(r => recordsByEmp.set(r.employee_id, r));

        const rows = employeesRes.data.map(emp => {
          const record = recordsByEmp.get(emp.id);
          const sessions = record?.sessions || [];
          const { checkIn, checkOut, isActive } = parseSessions(sessions);
          const status = record?.status || statusMap.get(emp.id) || 'absent';
          return {
            id: emp.id,
  ...
  }, [month, year, adminDate, isEmployee, isAdminView]);
    ...
    <div className="page">
      ...
      <div className="table-card">
        <table className="table">
          ...
          <tbody>
            {(isEmployee ? records : adminRows).map(r => (
              <tr key={r.id}>
                {!isEmployee && (
                  <>
                ...
                <td>
                  <span className="checkout-time">
                    {r.is_active && !r.check_out_time ? 'In Progress' : fmtTime(r.check_out_time)}
                  </span>
                </td>
                <td>
        ...
        {(isEmployee ? records : adminRows).length === 0 && (
  ...
  return (
    <div className={`attendance-today-card ${isActive ? 'attendance-today-card--active' : ''} ${checkOutTime ? 'attendance-today-card--done' : ''}`}>
      ...
      <div className="attendance-today-card__right">
        {!isActive ? (
          <button
            className="btn btn--success btn--lg attendance-check-btn"
            onClick={onCheckIn}
          >
